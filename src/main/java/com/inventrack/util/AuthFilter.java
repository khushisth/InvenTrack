...
public class AuthFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        boolean buyerForbiddenPath = path.startsWith(contextPath + "/dashboard") || 
                                     path.startsWith(contextPath + "/pos") || 
                                     path.startsWith(contextPath + "/users") || 
                                     path.startsWith(contextPath + "/products") || 
                                     path.startsWith(contextPath + "/categories") || 
                                     path.startsWith(contextPath + "/suppliers");
        ...
        if (loggedIn) {
            ...
            if ("Buyer".equalsIgnoreCase(role) && buyerForbiddenPath) {
                res.sendError(HttpServletResponse.SC_FORBIDDEN, "Buyers do not have access to this page.");
                return;
            }

            if (stockManagerOnlyPath && !"Stock Manager".equalsIgnoreCase(role)) {
